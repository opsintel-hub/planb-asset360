...
import { getDashboardOverview } from "@/lib/data.functions";
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="relative">
          ...
          <p className="mt-2 text-primary-foreground/80 max-w-2xl">
            ข้อมูลจริงจากฐานข้อมูล Lovable Cloud (Postgres) — กด "ทดสอบการ Sync" ในหน้าตั้งค่าเพื่อดึงข้อมูลจาก PlanB API ครั้งแรก
          </p>
      ...
      {isLoading || !data ? (
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard label="ทรัพย์สินทั้งหมด" value={data.stats.totalAssets.toLocaleString()} delta="ป้ายโฆษณาในระบบ" icon={<Activity className="size-5" />} tone="default" />
            <StatCard label="PM เดือนนี้" value={String(data.stats.pmThisMonth)} delta="งานบำรุงรักษา" icon={<CheckCircle2 className="size-5" />} tone="success" />
            <StatCard label="Claim ค้างอยู่" value={String(data.stats.claimsOpen)} delta={`Avg. ${data.stats.avgClaimDays} วันต่อรายการ`} icon={<Wrench className="size-5" />} tone="warning" />
            <StatCard label="Error Monitoring" value={String(data.stats.monitorErrors)} delta="ต้องดำเนินการด่วน" icon={<AlertTriangle className="size-5" />} tone="danger" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 rounded-xl border bg-card p-5 shadow-[var(--shadow-card)]">
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">PM, Claim, Monitoring ใน 5 เดือนล่าสุด</p>
                </div>
            ...
            <div className="rounded-xl border bg-card p-5 shadow-[var(--shadow-card)]">
              ...
              <p className="text-xs text-muted-foreground mb-4">สัดส่วนของทรัพย์สินตามสถานะ</p>
              {data.statusData.length === 0 ? (
                <div className="h-[220px] grid place-items-center text-sm text-muted-foreground">ยังไม่มีข้อมูล</div>
              ) : (
                <>
                  <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
                      <Pie data={data.statusData} dataKey="value" innerRadius={55} outerRadius={85} paddingAngle={3}>
                        {data.statusData.map((d, i) => <Cell key={i} fill={d.color} />)}
                      </Pie>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div className="lg:col-span-2 rounded-xl border bg-card p-5 shadow-[var(--shadow-card)]">
              ...
              {data.recent.length === 0 ? (
                <div className="py-10 text-center text-sm text-muted-foreground">ยังไม่มีข้อมูล — ลอง Sync ในหน้าตั้งค่า</div>
              ) : (
                <div className="divide-y">
                  {data.recent.slice(0, 6).map((r) => (
                    <div key={String(r.id)} className="py-3 flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <Badge tone={r.type === "Claim" ? "warning" : r.type === "Monitor" ? "danger" : "info"}>{r.status}</Badge>
                        </div>
                        <div className="text-xs text-muted-foreground mt-0.5">
                          {r.asset_old_code ?? r.id} · {r.type} · {r.opened_at ? new Date(r.opened_at).toLocaleString("th-TH") : ""}
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            <div className="rounded-xl border bg-card p-5 shadow-[var(--shadow-card)]">
              ...
              {data.deptData.length === 0 ? (
                <div className="h-[260px] grid place-items-center text-sm text-muted-foreground">ยังไม่มีข้อมูล</div>
              ) : (
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={data.deptData} layout="vertical" margin={{ left: 8 }}>
                    ...
                    <YAxis type="category" dataKey="name" stroke="oklch(0.5 0.03 250)" fontSize={12} width={70} />
                    <Tooltip contentStyle={{ borderRadius: 8 }} />
