# Roadmap

- [x] Add a live 360-day Claim and PM history summary for each asset.
- [x] Redesign the selected asset detail as a compact 16:9 executive report.
- [x] Remove the cumulative claim trend and replace generic advice with accountable actions.
- [x] Verify types and the authenticated preview.
- [x] Add an editable PowerPoint export for the selected 16:9 risk report.
- [ ] Add Project and Media Type columns to the filtered risk list.
- [ ] Add live filtered-count and risk-by-project/department summary cards.
- [ ] Add a 16:9 overview/cover PowerPoint export for the current filtered result.
- [ ] Refine the risk workspace and selected-asset report alignment, then verify desktop rendering and exports.
