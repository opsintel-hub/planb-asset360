...
SELECT DISTINCT ON (h.asset_old_code, (h.payload ->> 'CreatedDate'), (h.payload ->> 'Category'))
  COALESCE(h.ref_number, h.id::text) AS ref_number,
  h.asset_old_code,
  NULLIF(h.payload ->> 'CreatedDate', '')::timestamptz AS created_at,
  NULLIF(h.payload ->> 'UpdatedDate', '')::timestamptz AS updated_at,
  COALESCE(NULLIF(h.payload ->> 'UpdatedDate', '')::timestamptz, NULLIF(h.payload ->> 'CreatedDate', '')::timestamptz, h.action_date) AS event_ts,
  h.payload ->> 'Category' AS category,
  CASE
    WHEN (h.payload ->> 'Category') = 'Claim' THEN 'Claim'
    WHEN (h.payload ->> 'Category') LIKE 'PM%' THEN 'PM'
    ELSE 'Other'
  END AS type,
  ...
  COALESCE((h.payload ->> 'TotalTurnaroundTime')::numeric, 0::numeric) AS total_turnaround_time,
  a.department AS asset_department,
  a.payload ->> 'MediaType' AS asset_media_type
FROM mssql_asset_history h
LEFT JOIN assets a ON a.old_code = h.asset_old_code
WHERE ((h.payload ->> 'Category') LIKE 'PM%' OR (h.payload ->> 'Category') = 'Claim')
  AND h.asset_old_code IS NOT NULL
  AND NULLIF(h.payload ->> 'CreatedDate', '') IS NOT NULL
ORDER BY h.asset_old_code, (h.payload ->> 'CreatedDate'), (h.payload ->> 'Category'),
  (NULLIF(h.payload ->> 'UpdatedDate', '')::timestamptz) DESC NULLS LAST,
  h.action_date DESC NULLS LAST, h.id DESC;
...
CREATE INDEX mv_pm_history_created_idx ON public.mv_pm_history (created_at);
...
WITH pm AS (
  ...
  SELECT asset_old_code, ref_number AS claim_ref,
    ...
    bkk_upc, project, problem_category, problem_detail, problem_equipment,
    solution_category, solution_detail
  FROM mv_pm_history
  WHERE type = 'Claim' AND COALESCE(created_at, event_ts) IS NOT NULL
), joined AS (
  SELECT pm.*, c.claim_ref, c.claim_ts, c.bkk_upc AS claim_bkk_upc,
    c.project AS claim_project, c.problem_category, c.problem_detail,
    c.problem_equipment, c.solution_category, c.solution_detail,
    row_number() OVER (PARTITION BY pm.asset_old_code, pm.pm_ref ORDER BY c.claim_ts) AS rn
  FROM pm LEFT JOIN claim c ON c.asset_old_code = pm.asset_old_code AND c.claim_ts >= pm.pm_end_ts
)
SELECT asset_old_code, pm_ref, pm_category, pm_end_ts, claim_ref, claim_ts,
  CASE WHEN claim_ts IS NOT NULL
    THEN GREATEST(1, ceil(EXTRACT(epoch FROM claim_ts - pm_end_ts) / 86400.0)::integer)
  ...
  problem_category, problem_detail, problem_equipment, solution_category, solution_detail
