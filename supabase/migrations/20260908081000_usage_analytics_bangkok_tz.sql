-- Align Usage Analytics reporting to Thailand time (Asia/Bangkok, UTC+7)
-- Previously all day/hour/week grouping used UTC, shifting charts by 7 hours.
...
BEGIN
  ...
  SELECT jsonb_build_object(
    'kpis', jsonb_build_object(
      ...
      'activeToday', (SELECT count(DISTINCT user_id) FROM base WHERE (created_at AT TIME ZONE 'Asia/Bangkok')::date = (now() AT TIME ZONE 'Asia/Bangkok')::date),
    ...
    'daily', COALESCE((SELECT jsonb_agg(x ORDER BY x->>'day') FROM (
        SELECT jsonb_build_object(
                 'day', to_char(date_trunc('day', created_at AT TIME ZONE 'Asia/Bangkok'), 'YYYY-MM-DD'),
                 'users', count(DISTINCT user_id),
                 'events', count(*)) AS x
        FROM ranged GROUP BY date_trunc('day', created_at AT TIME ZONE 'Asia/Bangkok')) t), '[]'::jsonb),
    'hourly', COALESCE((SELECT jsonb_agg(x ORDER BY (x->>'hour')::int) FROM (
        SELECT jsonb_build_object('hour', EXTRACT(hour FROM created_at AT TIME ZONE 'Asia/Bangkok')::int,
                                  'events', count(*),
                                  'users', count(DISTINCT user_id)) AS x
        FROM ranged GROUP BY EXTRACT(hour FROM created_at AT TIME ZONE 'Asia/Bangkok')) t), '[]'::jsonb),
    'weekly', COALESCE((SELECT jsonb_agg(x ORDER BY x->>'week') FROM (
        SELECT jsonb_build_object('week', to_char(date_trunc('week', created_at AT TIME ZONE 'Asia/Bangkok'), 'YYYY-MM-DD'),
                                  'users', count(DISTINCT user_id),
                                  'events', count(*)) AS x
        FROM ranged GROUP BY date_trunc('week', created_at AT TIME ZONE 'Asia/Bangkok')) t), '[]'::jsonb),
    'monthly', COALESCE((SELECT jsonb_agg(x ORDER BY x->>'month') FROM (
        SELECT jsonb_build_object('month', to_char(date_trunc('month', created_at AT TIME ZONE 'Asia/Bangkok'), 'YYYY-MM'),
                                  'users', count(DISTINCT user_id),
                                  'events', count(*)) AS x
        FROM ranged GROUP BY date_trunc('month', created_at AT TIME ZONE 'Asia/Bangkok')) t), '[]'::jsonb),
    ...
    'heatmap', COALESCE((SELECT jsonb_agg(x) FROM (
        SELECT jsonb_build_object('dow', EXTRACT(dow FROM created_at AT TIME ZONE 'Asia/Bangkok')::int,
                                  'hour', EXTRACT(hour FROM created_at AT TIME ZONE 'Asia/Bangkok')::int,
                                  'events', count(*)) AS x
        FROM ranged
        GROUP BY EXTRACT(dow FROM created_at AT TIME ZONE 'Asia/Bangkok'), EXTRACT(hour FROM created_at AT TIME ZONE 'Asia/Bangkok')) t), '[]'::jsonb),
    ...
    'alerts', jsonb_build_object(
      'unusualLogins', COALESCE((SELECT jsonb_agg(x) FROM (
          ...
          GROUP BY user_id, display_name, date_trunc('day', created_at AT TIME ZONE 'Asia/Bangkok')
...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'daily', COALESCE((SELECT jsonb_agg(x ORDER BY x->>'day') FROM (
        SELECT jsonb_build_object('day', to_char(date_trunc('day', started_at AT TIME ZONE 'Asia/Bangkok'), 'YYYY-MM-DD'),
                                  'minutes', round(sum(minutes), 1)) AS x
        FROM sess GROUP BY date_trunc('day', started_at AT TIME ZONE 'Asia/Bangkok')) t), '[]'::jsonb),
